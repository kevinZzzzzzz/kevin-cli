 ...
 * @param {String} t i18n翻译方法
 * @returns {String}
 */
export function getTimeState(t) {
  let timeNow = new Date();
  let hours = timeNow.getHours();
  if (hours >= 6 && hours <= 10) return `${t("views.login.6to10text")} ⛅`;
  if (hours >= 10 && hours <= 14) return `${t("views.login.10to14text")} 🌞`;
  if (hours >= 14 && hours <= 18) return `${t("views.login.14to18text")} 🌞`;
  if (hours >= 18 && hours <= 24) return `${t("views.login.18to24text")} 🌛`;
  if (hours >= 0 && hours <= 6) return `${t("views.login.0to6text")} 🌛`;
}
