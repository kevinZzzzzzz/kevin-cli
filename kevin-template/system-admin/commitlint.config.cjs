  ...
  plugins: [
    {
      ...
      rules: {
        "subject-examine": parsed => {
          if (!parsed.type) {
            return [false, "提交信息 type 不能为"];
          }
          if (!parsed.subject) {
            return [false, "提交信息 subject 不能为"];
          }
          const subjectArr = parsed.subject?.split(" ") || "";
          const containsStoryOrBugFlag = parsed.subject.includes("--story") || parsed.subject.includes("--bug");
